...
auto parser::parse_translation_unit_node() -> std::optional<translation_unit_syntax>
{
    ...
    while(check(token_kind::kw_import)) {
    ...
    while(not check(token_kind::eof)) {
        if (
            check(token_kind::kw_export)
            and peek(1).kind != token_kind::identifier
        ) {
            consume();
            report_current(
...
auto parser::parse_import_declaration() -> std::optional<import_syntax>
{
...
auto parser::parse_function() -> std::optional<function_id>
{
    auto exported = false;
    ...
    auto function = function_syntax {
        .full_span = combine_spans(full_start, arena.span(*body)),
        .exported = exported,
        .name = name->span,

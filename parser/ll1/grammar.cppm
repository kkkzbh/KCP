...
export struct [[nodiscard]] grammar_definition
{
    std::string start_symbol;
    std::vector<grammar_production> productions;
};

export struct [[nodiscard]] grammar_analysis
{
...
export auto cp_ll1_grammar() -> grammar_definition
{
        ...
        .productions =
            {
                ...
                production{"TranslationUnitPrefix",
                           {"kw_import", "ModuleName", "semicolon", "ImportListTail", "FunctionList"}},
                production{"TranslationUnitPrefix", {"identifier", "FunctionAfterName", "FunctionList"}},
                production{"TranslationUnitPrefix", {}},
                production{"TranslationUnitAfterExport",
                           {"kw_module", "ModuleName", "semicolon", "ImportList", "FunctionList"}},
                production{"TranslationUnitAfterExport", {"identifier", "FunctionAfterName", "FunctionList"}},
                production{"ImportList", {"kw_import", "ModuleName", "semicolon", "ImportList"}},
                production{"ImportList", {}},
                production{"ImportListTail", {"kw_import", "ModuleName", "semicolon", "ImportListTail"}},
                production{"ImportListTail", {}},
                production{"ModuleName", {"identifier", "ModuleNameTail"}},
                production{"ModuleNameTail", {"dot", "identifier", "ModuleNameTail"}},
                production{"ModuleNameTail", {}},
                ...
                production{"DeclarationStmt",
                           {"LetOrConst", "identifier", "DeclTypeOpt", "equal", "Expression", "semicolon"}},
                ...
                production{"DoWhileStmt",
                           {"kw_do", "Block", "kw_while", "l_paren", "Expression", "r_paren", "semicolon"}},
                production {
                    "ForStmt",
                    {"kw_for", "ForLabelOpt", "l_paren", "ForBinding", "colon", "Expression", "r_paren", "Block"}},
                ...
                production{"Type", {"identifier", "TypeArgumentOpt", "TypeConstOpt", "PointerSuffixList", "ReferenceOpt"}},
                ...
                production{"PointerSuffixList", {"type_star", "PointerSuffixList"}},
                production{"PointerSuffixList", {}},
                production{"ReferenceOpt", {"type_amp"}},
                production{"ReferenceOpt", {}},
                ...
                production{"PrimaryNoBrace", {"identifier"}},
...
export auto analyze_grammar(grammar_definition const& grammar) -> grammar_analysis
{
    auto epsilon = std::string{epsilon_symbol()};
    auto eof_marker = std::string{end_marker_symbol()};
    auto nonterminals = std::set<std::string>{};
    for(auto const& production : grammar.productions)
    {
        nonterminals.insert(production.lhs);
    ...
    while(changed)
    {
        ...
        for(auto const& production : grammar.productions)
        {
            ...
            auto first = first_of_sequence(grammar, analysis.first_sets, production.rhs);
    ...
    while(changed)
    {
        ...
        for(auto const& production : grammar.productions)
        {
            for(auto index : std::views::iota(0uz, production.rhs.size()))
            {
                ...
                auto beta = std::vector<std::string> (
                    production.rhs.begin() + static_cast<std::ptrdiff_t>(index + 1),
                    production.rhs.end());

                auto beta_first = first_of_sequence(grammar, analysis.first_sets, beta);
                auto& follow = analysis.follow_sets[symbol];
                auto before = follow.size();
    ...
    for(auto index : std::views::iota(0uz, grammar.productions.size()))
    {
        ...
        grouped[production.lhs].emplace_back(index, select);
    }

    for(auto const& [lhs, items] : grouped)
    {
        for(auto left : std::views::iota(0uz, items.size()))
        {
            for(auto right : std::views::iota(left + 1, items.size()))
            {
                auto overlap = std::vector<std::string>{};
                std::ranges::set_intersection(items[left].second, items[right].second, std::back_inserter(overlap));
                if(not overlap.empty())
                {
                    analysis.conflicts.push_back(std::format (
                        "{} conflict between production {} and {} on {}", lhs, items[left].first, items[right].first,
                        std::accumulate(std::next(overlap.begin()), overlap.end(), overlap.front(),

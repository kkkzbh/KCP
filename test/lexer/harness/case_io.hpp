    ...
    std::string text) -> std::string
{
    ...
    if (should_trim_final_newline and text.ends_with('\n')) {
        text.pop_back();
    }
...
inline auto parse_flags(std::string const& field) -> token_flags
{
    ...
    while (start <= field.size()) {
        auto const index = field.find(',', start);
        auto const part = index == std::string::npos
            ? field.substr(start)
            : field.substr(start, index - start);

        if (part == "leading_space") {
            flags |= token_flags::leading_space;
        } else if (part == "start_of_line") {
            flags |= token_flags::start_of_line;
        } else if (part == "unterminated") {
            flags |= token_flags::unterminated;
        } else if (part == "recovered") {
    ...
    -> std::vector<lexer_case>
{
    ...
    for (auto const& entry : std::filesystem::recursive_directory_iterator{root}) {
        if (entry.is_regular_file() and entry.path().extension() == ".lex") {
    ...
    for (auto const& source_path : paths) {
        auto const tokens_path = source_path.parent_path() / (source_path.stem().string() + ".tokens.jsonl");
        auto const diag_path = source_path.parent_path() / (source_path.stem().string() + ".diag.jsonl");

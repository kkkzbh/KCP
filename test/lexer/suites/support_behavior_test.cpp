...
auto main() -> int
{
    ...
    test_lexer::assert_true(
        sources.position(alpha, 0) == front::source_position{.line = 1, .column = 1},
        "position should report first character");
    test_lexer::assert_true(
        sources.position(alpha, return_offset) == front::source_position{.line = 2, .column = 3},
        "position should track multi-line offsets");
    test_lexer::assert_true(
        sources.position(alpha, x_offset) == front::source_position{.line = 2, .column = 10},
        "position should report later columns");
    test_lexer::assert_true(
        sources.position(alpha, file_text.size()) == front::source_position{.line = 3, .column = 1},
        "position should allow eof");
    test_lexer::assert_true(
        sources.position(alpha, file_text.size() + 4) == front::source_position{.line = 3, .column = 1},
        "position should clamp offsets past eof");

    auto const return_span = front::span{
        .file = alpha,
        .start = return_offset,
        .end = return_offset + std::string_view{"return"}.size(),
    };
    test_lexer::assert_true(sources.slice(return_span) == "return", "slice should extract substrings");

    auto default_diagnostic = front::diagnostic{};
    test_lexer::assert_true(default_diagnostic.message.empty(), "diagnostic message should default construct");

    auto* deleted_through_base = static_cast<front::diagnostic_sink*>(new front::vector_diagnostic_sink{});
    delete deleted_through_base;

    std::unique_ptr<front::diagnostic_sink> sink = std::make_unique<front::vector_diagnostic_sink>();
    sink->report(front::diagnostic{
        .code = front::diagnostic_code::invalid_character,
        .message = "invalid character",
        .primary_span = return_span,
    });

    auto* concrete_sink = dynamic_cast<front::vector_diagnostic_sink*>(sink.get());
    ...
    for (auto const kind : test_lexer::all_token_kinds) {
        auto const text = front::to_string(kind);
        test_lexer::assert_true(text != "unknown", "known token kinds should have spellings");
    }

    test_lexer::assert_true(front::to_string(front::token_kind::kw_let) == "kw_let",
        "to_string should match keyword tokens");
    test_lexer::assert_true(front::to_string(front::token_kind::arrow) == "arrow",
        "to_string should match punctuator tokens");
    test_lexer::assert_true(front::to_string(static_cast<front::token_kind>(255)) == "unknown",

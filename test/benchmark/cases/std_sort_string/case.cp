...
checked_checksum(values: vector<str> const&) -> i64
{
    let total: i64 = 0;
    let index: usize = 0;
    while(index < values.size()) {
        ...
        total = (total + word_score(values[index]) * ((index % 251 + 1) as i64)) % 1000000007;
        index += 1;
    }
    return total;
}
...
main() -> i32
{
    ...
    let original = vector<str>{};
    original.reserve(count as usize);
    let index = 0;
    while(index < count) {
        original.push_back(word_at((index * 4827 + count * 97) % 1000003));
        index += 1;
    }
    ...
    while(round < repeats) {
        let values = original;
        if(stable) {
            stable_sort(values, f(left: str const&, right: str const&) -> weak_ordering {
                return left <=> right;
            });
        } else {
            sort(values, f(left: str const&, right: str const&) -> weak_ordering {
                return left <=> right;
            });
        }

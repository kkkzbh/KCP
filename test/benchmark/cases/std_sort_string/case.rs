...
fn word_at(value: i32) -> &'static str {
    const WORDS: [&str; 32] = [
    ...
    WORDS[(value % WORDS.len() as i32) as usize]
}
...
fn checked_checksum(values: &[&'static str]) -> i64 {
    let mut total = 0i64;
...
fn main() {
    ...
    let mut original = Vec::<&'static str>::with_capacity(count as usize);
    let mut index = 0i32;
    while index < count {

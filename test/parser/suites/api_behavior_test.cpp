        ...
        R"(main()
{
    ...
    test_parser::assert_true(!parsed.trace.empty(), "trace should be populated when enabled");
    ...
    test_parser::assert_true(
        to_string(trace_event_kind::enter) == "enter"
            && to_string(trace_event_kind::match) == "match"
            && to_string(trace_event_kind::fail) == "fail"
            && to_string(trace_event_kind::exit) == "exit",
        ...
        R"(main()
{
    ...
    test_parser::assert_true(
        shaped.syntax_tree->functions.size() == 1 && shaped.syntax_tree->functions.front().body != nullptr,
    ...
    test_parser::assert_true(
        shaped_body[2]->expressions.front()->kind == expr_syntax_kind::array_literal
            && shaped_body[2]->expressions.front()->operands.empty(),
    ...
    test_parser::assert_true(
        shaped_body[3]->expressions.front()->kind == expr_syntax_kind::sequence_literal
            && shaped_body[3]->expressions.front()->operands.empty(),
    ...
    test_parser::assert_true(
        shaped_body[4]->kind == statement_syntax_kind::expr_stmt
            && shaped_body[4]->expressions.front()->kind == expr_syntax_kind::unary
            && shaped_body[4]->expressions.front()->operator_kind == token_kind::plus_plus,
        "postfix increment should remain an expression statement with unary ++");
    test_parser::assert_true(
        shaped_body[5]->kind == statement_syntax_kind::expr_stmt
            && shaped_body[5]->expressions.front()->kind == expr_syntax_kind::unary
            && shaped_body[5]->expressions.front()->operator_kind == token_kind::minus_minus,
        "postfix decrement should remain an expression statement with unary --");
    test_parser::assert_true(
        shaped_body[6]->kind == statement_syntax_kind::if_stmt
            && shaped_body[6]->statements.size() == 2
            && shaped_body[6]->statements[1]->kind == statement_syntax_kind::if_stmt,
        "else-if chains should stay nested as if statements");
    test_parser::assert_true(
        shaped_body[6]->statements[1]->statements.size() == 2
            && shaped_body[6]->statements[1]->statements[1]->kind == statement_syntax_kind::block,
    ...
    test_parser::assert_true(!rejected.accepted, "lexically invalid source should be rejected");
    test_parser::assert_true(!rejected.lexer_diagnostics.empty(), "lexically invalid source should keep lexer diagnostics");

...
```cp
impl option_i32 {
    is_some(self: option_i32 const&) -> bool
    {
        return match self {
            .some(v) => true,
            .none => false,
...
泛型 `variant` 的布局按具体实例计算。`optional<i32>`、`optional<str>`、`result<i32,str>` 分别拥有自己的 tag 和 payload storage；不同实例之间不共享运行时布局要求。

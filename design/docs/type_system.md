...
本文档记录当前主线编译器已经设计并准备实现的类型规则。范围严格对齐当前 parser 支持的语言面：函数、模块、`let` / `const`、内建类型、`array` / `tuple`、引用、指针、类型转换、流程控制和运算符。
...
`bool` 和 `char` 当前按 1 字节基础类型处理。整数字面量默认类型为 `i32`，浮点字面量默认类型为 `f64`。`true` 和 `false` 的类型为 `bool`，字符字面量的类型为 `char`，字符串字面量的类型为 `str`。

字符串字面量支持 `\n` 等转义字符，类型为 `str`。`str` 表示字符串视图，语义上接近 C++ 的 `std::string_view`。

内部存在 `unit` 类型，用于表示没有值的返回结果；它 lowered 到 LLVM IR 时是 `void`。`unit` 当前不是用户可写类型。

## 函数返回类型

函数声明和定义写作 `name(...) -> type { ... }`。函数本身当前不作为用户可写类型；闭包尚未进入主线 semantic 实现。
...
`array` 是同构类型，所有元素必须具有统一元素类型，长度 `N` 必须精确匹配。
...
没有上下文时，空 `[]` 报错，因为无法推导元素类型。`{ ... }` 当前不是表达式，不能用于变量初始化；后续 braced initializer 或 struct literal 语义需要重新设计。
...
范围表达式必须是 `array<T,N>`，循环变量类型为元素类型。`break` 和 `continue` 必须位于循环中；带 label 时，label 必须能解析到外层带标签的 `for`。

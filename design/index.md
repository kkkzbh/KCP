...
- [流程控制](/docs/flow)：`if`、循环、标签、`break` / `continue`、`return`、表达式语句、块表达式和正常完成判断。
- [结构](/docs/struct)：`struct / impl`、构造、析构、成员函数、关联函数、UFCS 和字段访问。
- [Enum](/docs/enum)：强类型整数枚举、作用域 case 和显式底层整数转换。
- [Variant](/docs/variant)：名义类型、case 构造、`match`、tag + payload storage 布局和 payload 析构限制。
